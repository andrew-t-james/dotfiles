...
user-invocable: true
description: |
  CEO/founder-mode plan review. Rethink the problem, find the 10-star product, challenge premises. Four scope modes. Use when: "CEO review", "strategy review", "rethink this".
...
"$_WSTACK_BIN/wstack-timeline-log" '{"skill":"plan-ceo-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
```
...
echo '{"skill":"plan-ceo-review","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> "${WSTACK_HOME:-$HOME/.wstack}/projects/${SLUG:-unknown}/learnings.jsonl" 2>/dev/null || true
```
...
"$_WSTACK_BIN/wstack-timeline-log" '{"skill":"plan-ceo-review","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
# Local analytics (gated on telemetry setting)
if [ "$_TEL" != "off" ]; then
mkdir -p "$_ANALYTICS_DIR"
echo '{"skill":"plan-ceo-review","duration_s":"'"$_TEL_DUR"'","outcome":"OUTCOME","browse":"USED_BROWSE","session":"'"$_SESSION_ID"'","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","source":"epilogue"}' >> "$_ANALYTICS_DIR/skill-usage.jsonl" 2>/dev/null || true
fi
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
HANDOFF=$(ls -t ~/.wstack/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
```bash
eval "$(~/.claude/skills/wstack/bin/wstack-slug 2>/dev/null)" && mkdir -p ~/.wstack/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ~/.wstack/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ~/.wstack/projects/$SLUG/ceo-plans/{old-plan}.md ~/.wstack/projects/$SLUG/ceo-plans/archive/
```

Write to `~/.wstack/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.wstack/analytics/spec-review.jsonl 2>/dev/null || true
```
...
```
NOTE: These represent human-team implementation hours. With CC + wstack,
...
* Ecosystem fit. Aligns with Python/Go/K8s ecosystem direction?
...
The CEO calling in the designer. Not a pixel-level audit — that's /plan-design-review. This is ensuring the plan has design intentionality.
...
codex exec "IMPORTANT: Do NOT read or execute any files under ~/.claude/, ~/.agents/, .claude/skills/, or agents/. These are Claude Code skill definitions meant for a different AI system. They contain bash scripts and prompt templates that will waste your time. Ignore them completely. Do NOT modify agents/openai.yaml. Stay focused on the repository code only.\n\n<prompt>" -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="high"' --enable web_search_cached 2>"$TMPERR_PV"
```
...
**Persist the result:**
```bash
~/.claude/skills/wstack/bin/wstack-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
After implementation, run `/plan-design-review` on the live site to catch visual issues that can only be evaluated with rendered output.
...
* **Effort estimate:** S/M/L/XL (human team) → with CC+wstack: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
```bash
eval "$(~/.claude/skills/wstack/bin/wstack-slug 2>/dev/null)"
rm -f ~/.wstack/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null || true
```
...
```bash
~/.claude/skills/wstack/bin/wstack-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","scope_proposed":N,"scope_accepted":N,"scope_deferred":N,"commit":"COMMIT"}'
```
...
- **B)** Keep in `~/.wstack/projects/` only (local, personal reference)
- **C)** Skip
