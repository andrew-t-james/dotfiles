...
user-invocable: true
description: |
  Eng manager plan review: architecture, data flow, edge cases, test coverage, performance. Interactive with recommendations. Use when: "eng review", "architecture review".
...
"$_WSTACK_BIN/wstack-timeline-log" '{"skill":"plan-eng-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
```
...
echo '{"skill":"plan-eng-review","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> "${WSTACK_HOME:-$HOME/.wstack}/projects/${SLUG:-unknown}/learnings.jsonl" 2>/dev/null || true
```
...
"$_WSTACK_BIN/wstack-timeline-log" '{"skill":"plan-eng-review","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
# Local analytics (gated on telemetry setting)
if [ "$_TEL" != "off" ]; then
mkdir -p "$_ANALYTICS_DIR"
echo '{"skill":"plan-eng-review","duration_s":"'"$_TEL_DUR"'","outcome":"OUTCOME","browse":"USED_BROWSE","session":"'"$_SESSION_ID"'","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","source":"epilogue"}' >> "$_ANALYTICS_DIR/skill-usage.jsonl" 2>/dev/null || true
fi
...
After producing the coverage diagram, write a test plan artifact to the project directory so `/qa` can consume it as primary test input:
...
Write to `~/.wstack/projects/{slug}/{user}-{branch}-eng-review-test-plan-{datetime}.md`:
...
Analyze the plan's implementation steps for parallel execution opportunities. This helps split work across git worktrees (via Claude Code's Agent tool with `isolation: "worktree"` or parallel workspaces).
...
```bash
~/.claude/skills/wstack/bin/wstack-review-log '{"skill":"plan-eng-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"MODE","commit":"COMMIT"}'
```
