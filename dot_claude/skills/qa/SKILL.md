...
user-invocable: true
description: |
  QA test + fix loop. Tests a web app, finds bugs, fixes them with atomic commits, re-verifies. Use when: "qa", "test this site", "find and fix bugs".
allowed-tools:
  - Bash
  - Read
  - Write
...
"$_WSTACK_BIN/wstack-timeline-log" '{"skill":"qa","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
```
...
echo '{"skill":"qa","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> "${WSTACK_HOME:-$HOME/.wstack}/projects/${SLUG:-unknown}/learnings.jsonl" 2>/dev/null || true
```
...
"$_WSTACK_BIN/wstack-timeline-log" '{"skill":"qa","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
# Local analytics (gated on telemetry setting)
if [ "$_TEL" != "off" ]; then
mkdir -p "$_ANALYTICS_DIR"
echo '{"skill":"qa","duration_s":"'"$_TEL_DUR"'","outcome":"OUTCOME","browse":"USED_BROWSE","session":"'"$_SESSION_ID"'","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","source":"epilogue"}' >> "$_ANALYTICS_DIR/skill-usage.jsonl" 2>/dev/null || true
fi
...
| Mode | full | `--regression .wstack/qa-reports/baseline.json` |
| Output dir | `.wstack/qa-reports/` | `Output to /tmp/qa` |
...
```bash
mkdir -p .wstack/qa-reports/screenshots
```
...
1. **Project-scoped test plans:** Check `~/.wstack/projects/` for recent `*-test-plan-*.md` files for this repo
   ```bash
   eval "$(~/.claude/skills/wstack/bin/wstack-slug 2>/dev/null)"
setopt +o nomatch 2>/dev/null || true  # zsh compat
   ls -t ~/.wstack/projects/$SLUG/*-test-plan-*.md 2>/dev/null | head -1
   ```
...
- Include full attribution comment:
  ...
  // Report: .wstack/qa-reports/qa-report-{domain}-{date}.md
  ```
...
**Local:** `.wstack/qa-reports/qa-report-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:** Write test outcome artifact for cross-session context:
```bash
eval "$(~/.claude/skills/wstack/bin/wstack-slug 2>/dev/null)" && mkdir -p ~/.wstack/projects/$SLUG
```
Write to `~/.wstack/projects/{slug}/{user}-{branch}-test-outcome-{datetime}.md`
