...
version: 2026.04.13
description: |
  Ship workflow: merge base, tests, review, VERSION bump, CHANGELOG, commit, push, create PR. Use when: "ship", "create PR", "push to main". Auto-suggested when code is ready.
allowed-tools:
  - Bash
  - Read
  - Write
...
"$_WSTACK_BIN/wstack-timeline-log" '{"skill":"ship","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
```
...
echo '{"skill":"ship","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> "${WSTACK_HOME:-$HOME/.wstack}/projects/${SLUG:-unknown}/learnings.jsonl" 2>/dev/null || true
```
...
"$_WSTACK_BIN/wstack-timeline-log" '{"skill":"ship","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
# Local analytics (gated on telemetry setting)
if [ "$_TEL" != "off" ]; then
mkdir -p "$_ANALYTICS_DIR"
echo '{"skill":"ship","duration_s":"'"$_TEL_DUR"'","outcome":"OUTCOME","browse":"USED_BROWSE","session":"'"$_SESSION_ID"'","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","source":"epilogue"}' >> "$_ANALYTICS_DIR/skill-usage.jsonl" 2>/dev/null || true
fi
...
For Design Review: check if the diff touches frontend files:
```bash
SCOPE_FRONTEND=false
git diff origin/<base> --name-only | grep -qE '\.(css|scss|less|html|erb|haml|slim|jsx|tsx|vue|svelte)$' && SCOPE_FRONTEND=true
echo "SCOPE_FRONTEND=$SCOPE_FRONTEND"
```
If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /plan-design-review for a full visual audit post-implementation." Still never block.
...
[ -f .wstack/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
```
...
If user picks H → write `.wstack/no-test-bootstrap` and continue without tests.
...
If user picks C → write `.wstack/no-test-bootstrap`. Tell user: "If you change your mind later, delete `.wstack/no-test-bootstrap` and re-run." Continue without tests.
...
- Create an issue assigned to that person (use the platform detected in Step 0):
  - **If GitHub:**
    ...
    gh issue create \
      ...
      --body "Found failing on branch <current-branch>. Failure is pre-existing.\n\n**Error:**\n```\n<first 10 lines>\n```\n\n**Last modified by:** <author>\n**Noticed by:** wstack /ship on <date>" \
...
Write to `~/.wstack/projects/{slug}/{user}-{branch}-ship-test-plan-{datetime}.md`:
...
for PLAN_DIR in "$HOME/.claude/plans" "$HOME/.codex/plans" ".wstack/plans"; do
  [ -d "$PLAN_DIR" ] || continue
...
- Review report sections (`## WSTACK REVIEW REPORT`)
...
Automatically verify the plan's testing/verification steps using the `/qa` skill.
...
4. **Apply the design checklist** against the changed files. For each item:
   ...
   - **[LOW] intent-based detection**: present as "Possible — verify visually or run /plan-design-review"
...
```bash
~/.claude/skills/wstack/bin/wstack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
~/.claude/skills/wstack/bin/wstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
which codex 2>/dev/null && echo "CODEX_AVAILABLE" || echo "CODEX_NOT_AVAILABLE"
# Respect old opt-out
OLD_CFG=$(~/.claude/skills/wstack/bin/wstack-config get codex_reviews 2>/dev/null || true)
...
```bash
~/.claude/skills/wstack/bin/wstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"medium","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
~/.claude/skills/wstack/bin/wstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"large","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
# Detect diff scope
SCOPE_FRONTEND=false; SCOPE_BACKEND=false; SCOPE_AUTH=false; SCOPE_MIGRATIONS=false; SCOPE_API=false
_CHANGED_FILES=$(git diff origin/<base> --name-only)
echo "$_CHANGED_FILES" | grep -qE '\.(css|scss|less|html|erb|haml|slim|jsx|tsx|vue|svelte)$' && SCOPE_FRONTEND=true
echo "$_CHANGED_FILES" | grep -qE '\.(rb|py|go|rs|java|php)$' && SCOPE_BACKEND=true
echo "$_CHANGED_FILES" | grep -qiE '(auth|session|login|password|oauth|token)' && SCOPE_AUTH=true
echo "$_CHANGED_FILES" | grep -qE '(migrate|migration)' && SCOPE_MIGRATIONS=true
echo "$_CHANGED_FILES" | grep -qiE '(api/|controllers/api|routes/api|endpoints)' && SCOPE_API=true
echo "SCOPE_FRONTEND=$SCOPE_FRONTEND SCOPE_BACKEND=$SCOPE_BACKEND SCOPE_AUTH=$SCOPE_AUTH SCOPE_MIGRATIONS=$SCOPE_MIGRATIONS SCOPE_API=$SCOPE_API"
...
1. **Testing** — read `~/.claude/skills/wstack/review/specialists/testing.md`
2. **Maintainability** — read `~/.claude/skills/wstack/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `~/.claude/skills/wstack/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `~/.claude/skills/wstack/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `~/.claude/skills/wstack/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `~/.claude/skills/wstack/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `~/.claude/skills/wstack/review/design-checklist.md`
...
```bash
eval "$(~/.claude/skills/wstack/bin/wstack-slug 2>/dev/null)"
cat "${WSTACK_HOME:-$HOME/.wstack}/projects/${SLUG:-unknown}/learnings.jsonl" 2>/dev/null | grep "pitfall" | tail -5 || true
```
...
1. The red-team checklist from `~/.claude/skills/wstack/review/specialists/red-team.md`
...
Check for learnings from previous sessions:

```bash
eval "$(~/.claude/skills/wstack/bin/wstack-slug 2>/dev/null)"
_LEARN_FILE="${WSTACK_HOME:-$HOME/.wstack}/projects/${SLUG:-unknown}/learnings.jsonl"
[ -f "$_LEARN_FILE" ] && tail -10 "$_LEARN_FILE" || echo "NO_LEARNINGS"
```

If learnings are found, incorporate them into your analysis.
...
## Step 4: Version bump (CalVer auto-increment)

**Idempotency check:** Before bumping, compare VERSION against the base branch.

```bash
BASE_VERSION=$(git show origin/<base>:VERSION 2>/dev/null || echo "")
CURRENT_VERSION=$(cat VERSION 2>/dev/null || echo "")
echo "BASE: $BASE_VERSION  HEAD: $CURRENT_VERSION"
if [ -n "$CURRENT_VERSION" ] && [ "$CURRENT_VERSION" != "$BASE_VERSION" ]; then echo "ALREADY_BUMPED"; fi
```
...
**CalVer format:** `YYYY.MM.DD` with letter suffix for same-day releases (`2026.03.31` → `2026.03.31a` → `2026.03.31b`).

Manual CalVer bump:

1. Read the current `VERSION` file
2. Compute today's date as `YYYY.MM.DD`
3. If the current version already matches today's date, append/increment a letter suffix
...
5. **Write the CHANGELOG entry** covering ALL groups:
   ...
   - Format: `## [YYYY.MM.DD] - YYYY-MM-DD` (or `## [YYYY.MM.DDx] - YYYY-MM-DD` for same-day releases)
...
- Message: "wstack recommends maintaining a TODOS.md organized by skill/component, then priority (P0 at top through P4, then Completed at bottom). See TODOS-format.md for the full format. Would you like to create one?"
...
**4. Move completed items** to the `## Completed` section at the bottom. Append: `**Completed:** v{VERSION} (YYYY-MM-DD)`
...
chore: bump version and changelog (v{VERSION})
...
## Specialist Review
<If Review Army dispatched: "N specialists dispatched, M findings (X critical, Y informational). PR Quality Score: Z/10">
<If specialists skipped (small diff): omit this section>
...
## Step 8.25: Link PR to Linear (if configured)

If `LINEAR` is `configured` (from preamble output):

1. Find the Linear issue for this feature. Search in order:
   - Check wstack artifacts in `~/.wstack/projects/$SLUG/wstack/` for intake or scope files
     that have a `## Linear` section with an issue ID
   - If no artifact found, search Linear for issues matching the branch name or PR title
     using `mcp__claude_ai_Linear__search_issues`
   - If still not found, search by recent issues in the team's project

2. If a linked issue is found:
   - Add a comment with the PR URL: "PR created: {PR URL}"
   - Attach the PR URL to the issue using `mcp__claude_ai_Linear__create_attachment`
     with the PR URL and title
   - Update issue status to "In Review" (or equivalent)
   - Add label `wstack:pr-created`
   - Tell the user: "Linked PR to Linear issue {identifier}"

3. If no issue found: skip silently. Not every PR has a Linear issue.

If `LINEAR` is `not_configured`: skip silently.
...
Append to `~/.wstack/projects/$SLUG/$BRANCH-reviews.jsonl`:

```bash
echo '{"skill":"ship","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","coverage_pct":COVERAGE_PCT,"plan_items_total":PLAN_TOTAL,"plan_items_done":PLAN_DONE,"verification_result":"VERIFY_RESULT","version":"VERSION","branch":"BRANCH"}' >> ~/.wstack/projects/$SLUG/$BRANCH-reviews.jsonl
```
...
If you discovered a non-obvious pattern or pitfall, log it:

```bash
eval "$(~/.claude/skills/wstack/bin/wstack-slug 2>/dev/null)"
_LEARN_DIR="${WSTACK_HOME:-$HOME/.wstack}/projects/${SLUG:-unknown}"
mkdir -p "$_LEARN_DIR"
echo '{"skill":"ship","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> "$_LEARN_DIR/learnings.jsonl"
```
...
- **Never ask for trivial confirmations** (e.g., "ready to push?", "create PR?"). DO stop for: version bumps (MINOR/MAJOR), pre-landing review findings (ASK items), Codex structured review [P1] findings (large diffs only), and AI-assessed coverage below minimum threshold.
- **Always use CalVer format** (YYYY.MM.DD with optional letter suffix) from the VERSION file.
