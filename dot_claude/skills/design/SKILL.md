...
user-invocable: true
description: |
  Generate UI prototypes using WakeCap's Wakecore component library (80+ components). Use when: "design", "prototype", "build a UI", "mockup with Wakecore".
allowed-tools:
  - Bash
  - Read
  - Write
...
"$_WSTACK_BIN/wstack-timeline-log" '{"skill":"design","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
```
...
echo '{"skill":"design","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> "${WSTACK_HOME:-$HOME/.wstack}/projects/${SLUG:-unknown}/learnings.jsonl" 2>/dev/null || true
```
...
"$_WSTACK_BIN/wstack-timeline-log" '{"skill":"design","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
# Local analytics (gated on telemetry setting)
if [ "$_TEL" != "off" ]; then
mkdir -p "$_ANALYTICS_DIR"
echo '{"skill":"design","duration_s":"'"$_TEL_DUR"'","outcome":"OUTCOME","browse":"USED_BROWSE","session":"'"$_SESSION_ID"'","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","source":"epilogue"}' >> "$_ANALYTICS_DIR/skill-usage.jsonl" 2>/dev/null || true
fi
# Remote telemetry (opt-in, requires binary)
if [ "$_TEL" != "off" ] && [ -n "$_WSTACK_BIN" ] && [ -x "$_WSTACK_BIN/wstack-telemetry-log" ]; then
"$_WSTACK_BIN/wstack-telemetry-log" \
  --skill "design" --duration "$_TEL_DUR" --outcome "OUTCOME" \
...
# /design — WakeCap UI Prototyping

Generate UI prototypes using Wakecore components. The component catalog is
embedded in this skill, no GitHub access or npm registry needed.

**HARD GATE:** This skill generates PROTOTYPE code only. It does NOT modify
production source code or deploy anything. Output goes to `prototypes/` directory.

**This is NOT /sellit** (for existing product issues) or `/qa` (for testing).
This is for creating new UI prototypes with real Wakecore components.

---

## Step 0: Load Component Catalog

```bash
CATALOG="~/.claude/skills/wstack/design/wakecore-catalog.json"
[ -f "$CATALOG" ] && echo "CATALOG_FOUND" || echo "CATALOG_MISSING"
```

**If CATALOG_FOUND:** Read the full catalog with the Read tool.
Note the version:
```bash
~/.claude/skills/wstack/bin/wstack-design-sync --check
```

**If CATALOG_MISSING:** Tell the user:
"Wakecore catalog not found. Run `wstack-design-sync /path/to/Wakecore` to sync it."
Continue without catalog, but warn that component names may not match Wakecore.
...
> What UI do you want to build? Describe the page, feature, or component.
> Examples: "worker safety dashboard", "project overview with KPIs",
> "form for submitting incident reports", "settings page with tabs".

From their description, produce a **Design Intent** document:

1. **Purpose**: What problem does this UI solve? Who uses it?
2. **Key data**: What data does this page display or collect?
   Ask: "What real data fields does this page show? (e.g., worker name, check-in time, site location)"
   Do NOT assume mock data. Use the user's actual data model.
3. **User actions**: What can the user DO on this page?
4. **Information hierarchy**: What's most important? What's secondary?
5. **Constraints**: Mobile-first? Specific viewport? Accessibility requirements?

---

## Step 2: Component Selection

Based on the intent, search the catalog for matching components.

**Selection rules:**
- Match by tags first (e.g., "dashboard" → DataTable, Chart, Card, StatCard)
- Check subcomponents (e.g., Card has Header, Title, Content, Footer)
- Prefer Wakecore components over custom HTML. If a component exists in the catalog, use it.
- For layouts: use Sidebar, Tabs, Card for structure
- For data: use DataTable for tables, Chart for visualization
- For forms: use Form + FormField + Input/Select/DatePicker
- For feedback: use Toast, Alert, Empty for states

Produce a **Component List** showing which Wakecore components you'll use and why.
Present it to the user via AskUserQuestion:

> Here's the component plan. Does this match what you're thinking?
> {list of components and their roles}
> A) Looks good, generate the prototype
> B) I'd change some components (tell me which)
...
Generate a standalone prototype project:

```bash
PROTO_NAME=$(echo "{user's description}" | tr '[:upper:]' '[:lower:]' | tr ' ' '-' | tr -cd 'a-z0-9-' | head -c 30)
mkdir -p prototypes/$PROTO_NAME/src
echo "PROTOTYPE: prototypes/$PROTO_NAME/"
```

Create these files:

**prototypes/{name}/package.json:**
```json
{
  "name": "{name}",
  "private": true,
  ...
  "dependencies": {
    "@wakecap/core-ui": "latest",
    "@wakecap/core-tokens": "latest",
    "@wakecap/core-utils": "latest",
    "react": "^19.0.0",
    "react-dom": "^19.0.0"
  },
  "devDependencies": {
...
**prototypes/{name}/src/App.tsx** — the main component using Wakecore components.

**Import pattern:**
```typescript
import { Button } from "@wakecap/core-ui/button";
import { Card, CardHeader, CardTitle, CardContent } from "@wakecap/core-ui/card";
import { DataTable } from "@wakecap/core-ui/data/core-data-table";
import "@wakecap/core-tokens";
```

**Prototype rules:**
- Use ONLY components from the catalog. Never invent custom components.
- Use the exact import paths from the catalog.
- Use Tailwind utility classes for layout (flex, grid, gap, padding).
- Use Wakecore design tokens for colors (CSS variables: --primary, --muted, etc.).
- Include ALL interaction states: loading (Skeleton), empty (Empty), error (Alert).
- Make it responsive (mobile-first, Tailwind breakpoints).
- Use REAL data field names from the user's description. NOT mock data like
  "John Doe" or "Lorem ipsum." Use realistic WakeCap domain data (worker IDs,
  site names, check-in timestamps, safety zone names).
- If the user didn't specify data fields, ASK before generating.

**For users WITHOUT GitHub registry access:**
Tell them: "This prototype uses Wakecore component names. To run it, you need
access to the GitHub npm registry. Alternatively, replace `@wakecap/core-ui`
imports with shadcn/ui equivalents (same component names, similar APIs)."

---

## Step 4: Verify Prototype

**If browse ($B) is available:** render and verify the prototype.

```bash
cd prototypes/$PROTO_NAME
npm install 2>/dev/null && npm run dev &
sleep 3
$B goto http://localhost:5173
$B snapshot -a -o /tmp/prototype-desktop.png
$B viewport 375x812
$B snapshot -a -o /tmp/prototype-mobile.png
```

Show both screenshots to the user using Read tool.
Check:
1. Does the desktop layout look correct?
2. Does the mobile layout work (no overflow, readable)?
3. Are all interaction states visible (or accessible via navigation)?
...
**If browse is NOT available:** Skip visual verification. Tell the user:
"Prototype generated but not visually verified (browse not available).
Run `cd prototypes/{name} && npm install && npm run dev` to see it locally."

---

## Step 5: Design Review Checklist

Review the prototype against Wakecore standards:

| Check | Status |
|-------|--------|
| Token compliance (CSS variables, no hardcoded hex) | {pass/fail} |
| Component usage (correct props and variants) | {pass/fail} |
| Accessibility (forms labeled, keyboard nav) | {pass/fail} |
| Responsive (works at 375px mobile) | {pass/fail} |
| States (loading, empty, error handled) | {pass/fail} |
| Hierarchy (most important content prominent) | {pass/fail} |
| Real data (no mock "John Doe" placeholders) | {pass/fail} |

Fix any failures before presenting to the user.

---

## Step 6: Write Artifact + Next Steps

```bash
eval "$(~/.claude/skills/wstack/bin/wstack-slug 2>/dev/null)" && mkdir -p ~/.wstack/projects/$SLUG
mkdir -p ~/.wstack/projects/$SLUG/wstack
DATETIME=$(date +%Y%m%d-%H%M%S)
echo "ARTIFACT: ~/.wstack/projects/$SLUG/wstack/design-$DATETIME.md"
```
...
Wakecore Catalog: v{version} @ {sourceCommit}

## Intent
{purpose, data, actions, hierarchy}

## Components Used
{list with import paths}

## Verification
- Desktop: {pass/fail + screenshot ref}
- Mobile: {pass/fail + screenshot ref}
- Review checklist: {X}/7 passed

## Screenshots
- Desktop: /tmp/prototype-desktop.png
- Mobile: /tmp/prototype-mobile.png
```

**Suggest next step:**
- Prototype needs architecture review → `/autoplan`
- Prototype ready to build → create Linear issue, then `/ship` when code is ready
- Prototype needs validation → `/qa` to test, or share with the requesting stakeholder

Tell the user: "Prototype generated at `prototypes/{name}/`. Next: `/autoplan`
to validate the approach, or start building and `/ship` when ready."

---

## Catalog Updates

The Wakecore catalog is version-stamped. To update after a new Wakecore release:

```bash
~/.claude/skills/wstack/bin/wstack-design-sync /path/to/Wakecore
```
