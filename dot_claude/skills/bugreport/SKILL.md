...
user-invocable: true
description: |
  Structured bug capture for Field Engineers. Creates production-ready
  Linear issues with screenshots, repro steps, severity. Works without
  a git repo. Use when: "bug", "broken", "not working", "report issue".
allowed-tools:
  - Bash
  - Read
  - Write
...
"$_WSTACK_BIN/wstack-timeline-log" '{"skill":"bugreport","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
```
...
echo '{"skill":"bugreport","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> "${WSTACK_HOME:-$HOME/.wstack}/projects/${SLUG:-unknown}/learnings.jsonl" 2>/dev/null || true
```
...
"$_WSTACK_BIN/wstack-timeline-log" '{"skill":"bugreport","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
# Local analytics (gated on telemetry setting)
if [ "$_TEL" != "off" ]; then
mkdir -p "$_ANALYTICS_DIR"
echo '{"skill":"bugreport","duration_s":"'"$_TEL_DUR"'","outcome":"OUTCOME","browse":"USED_BROWSE","session":"'"$_SESSION_ID"'","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","source":"epilogue"}' >> "$_ANALYTICS_DIR/skill-usage.jsonl" 2>/dev/null || true
fi
# Remote telemetry (opt-in, requires binary)
if [ "$_TEL" != "off" ] && [ -n "$_WSTACK_BIN" ] && [ -x "$_WSTACK_BIN/wstack-telemetry-log" ]; then
"$_WSTACK_BIN/wstack-telemetry-log" \
  --skill "bugreport" --duration "$_TEL_DUR" --outcome "OUTCOME" \
...
# /bugreport — Field Engineer Bug Report

You are capturing a structured bug report from a WakeCap Field Engineer (or anyone
who found a bug). Your job is to collect enough detail that an Agentic Engineer can
assign this directly to a coding agent without further triage.

**Design goal:** Under 3 minutes, works without a git repo, works without browse.
...
```bash
git rev-parse --is-inside-work-tree 2>/dev/null && echo "IN_REPO=true" || echo "IN_REPO=false"
```

If in a repo, try to detect the product:

```bash
if [ -f CLAUDE.md ]; then head -20 CLAUDE.md; fi
if [ -f package.json ]; then grep '"name"' package.json | head -1; fi
```

Remember whether you are in a repo or not. If in a repo and you can identify the
product name from CLAUDE.md or package.json, remember it for Step 2.
...
**Q1: Which WakeCap product?** (skip if auto-detected from repo in Step 1)
...
- A) Portal (web app)
- B) Mobile App
- C) Hardware Dashboard
- D) Other (describe)

**Q2: What happened?**

> Describe the bug. What did you see? What error message appeared?

**Q3: What should have happened?**

> What were you expecting to see or happen instead?
...
Options:
- A) P0 Critical -- System down, data loss, safety risk, no workaround
- B) P1 High -- Major feature broken, affects many users, painful workaround
- C) P2 Medium -- Feature partially broken, workaround exists
- D) P3 Low -- Cosmetic, minor annoyance, does not block work

**Q6: URL or screenshot?** (optional)

> Do you have a URL where the bug occurs? Paste it, or describe the screen you were on. (Type "skip" if not applicable.)
...
If the user provided a URL AND browse (`$B`) is available:

```bash
$B goto "<url>"
$B screenshot /tmp/wstack-bugreport.png
$B console --errors
```

Show the screenshot to the user (use Read tool on the PNG). Note any console errors.

If browse is NOT available or the user did not provide a URL, skip this step.
Note: "Evidence: described verbally (no browse available)" or "Evidence: no URL provided."

---

## Step 4: Create Linear Issue

Try to create a Linear issue using `mcp__claude_ai_Linear__save_issue`.

Use this structure for the issue:

- **Title:** `[Bug] <short description from Q2>`
- **Description:**

```markdown
## Bug Report
**Product:** <product from Q1>
**Severity:** <P0-P3 from Q5>
**Reported by:** Field Engineer
**Date:** <today's date>
...
## Steps to reproduce
<numbered steps from Q4>

## Evidence
<screenshot reference if captured, console errors if captured, or "described verbally">

## Environment
<URL if provided, browser/device if mentioned, or "not specified">

---
_Created by wstack v$_WSTACK_VERSION — /bugreport_
```

- **Priority:** Map severity: P0 = 1 (Urgent), P1 = 2 (High), P2 = 3 (Medium), P3 = 4 (Low)
- **Labels:** `bug`, `field-reported`

If Linear MCP is not available, output the bug report as formatted Markdown and tell
the user: "Linear is not connected. Copy this report and paste it into a new Linear issue."
...
Show the user:
1. The Linear issue URL (if created), or the formatted Markdown (if not)
2. "Bug reported. An engineer can assign this directly to a coding agent."

If severity is P0 or P1, add: "This is high severity. Consider pinging the team in Slack."
