...
"$_WSTACK_BIN/wstack-timeline-log" '{"skill":"autoplan","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
```
...
echo '{"skill":"autoplan","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> "${WSTACK_HOME:-$HOME/.wstack}/projects/${SLUG:-unknown}/learnings.jsonl" 2>/dev/null || true
```
...
"$_WSTACK_BIN/wstack-timeline-log" '{"skill":"autoplan","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
# Local analytics (gated on telemetry setting)
if [ "$_TEL" != "off" ]; then
mkdir -p "$_ANALYTICS_DIR"
echo '{"skill":"autoplan","duration_s":"'"$_TEL_DUR"'","outcome":"OUTCOME","browse":"USED_BROWSE","session":"'"$_SESSION_ID"'","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","source":"epilogue"}' >> "$_ANALYTICS_DIR/skill-usage.jsonl" 2>/dev/null || true
fi
# Remote telemetry (opt-in, requires binary)
if [ "$_TEL" != "off" ] && [ -n "$_WSTACK_BIN" ] && [ -x "$_WSTACK_BIN/wstack-telemetry-log" ]; then
"$_WSTACK_BIN/wstack-telemetry-log" \
  --skill "autoplan" --duration "$_TEL_DUR" --outcome "OUTCOME" \
...
`~/.claude/skills/wstack/office-hours/SKILL.md`
...
```bash
SLUG=$(~/.claude/skills/wstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t ~/.wstack/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ~/.wstack/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
...
echo "RESTORE_PATH=$HOME/.wstack/projects/$SLUG/${BRANCH}-autoplan-restore-${DATETIME}.md"
```
...
- Discover design docs: `ls -t ~/.wstack/projects/$SLUG/*-design-*.md 2>/dev/null | head -1`
...
- `~/.claude/skills/wstack/plan-ceo-review/SKILL.md`
- `~/.claude/skills/wstack/plan-design-review/SKILL.md` (only if UI scope detected)
- `~/.claude/skills/wstack/plan-eng-review/SKILL.md`
- `~/.claude/skills/wstack/plan-devex-review/SKILL.md` (only if DX scope detected)
...
- Dual voices: always run BOTH Claude subagent AND Codex if available (P6).
  ...
  **Codex CEO voice** (via Bash):
  Command: `codex exec "IMPORTANT: Do NOT read or execute any files under ~/.claude/, ~/.agents/, .claude/skills/, or agents/. Stay focused on the repository code only. You are a CEO/founder advisor reviewing a development plan.
...
- Dual voices: always run BOTH Claude subagent AND Codex if available (P6).
  ...
  **Codex design voice** (via Bash):
  Command: `codex exec "IMPORTANT: Do NOT read or execute any files under ~/.claude/, ~/.agents/, .claude/skills/, or agents/. Stay focused on the repository code only. Read the plan file at <plan_path>. Evaluate this plan's
...
- Dual voices: always run BOTH Claude subagent AND Codex if available (P6).
  ...
  **Codex eng voice** (via Bash):
  Command: `codex exec "IMPORTANT: Do NOT read or execute any files under ~/.claude/, ~/.agents/, .claude/skills/, or agents/. Stay focused on the repository code only. Review this plan for architectural issues, missing edge cases,
...
- Test plan: generate artifact at `~/.wstack/projects/$SLUG/{user}-{branch}-test-plan-{datetime}.md`
...
- Dual voices: always run BOTH Claude subagent AND Codex if available (P6).
  ...
  **Codex DX voice** (via Bash):
  Command: `codex exec "IMPORTANT: Do NOT read or execute any files under ~/.claude/, ~/.agents/, .claude/skills/, or agents/. Stay focused on the repository code only. Read the plan file at <plan_path>. Evaluate this plan's developer experience.
  ...
  Be adversarial. Think like a developer who is evaluating this against 3 competitors." -C "$(git rev-parse --show-toplevel)" -s read-only --enable web_search_cached`
  Timeout: 10 minutes
...
- [ ] Test plan artifact written to disk at ~/.wstack/projects/$SLUG/
- [ ] "NOT in scope" section written
- [ ] "What already exists" section written
...
[If security/feasibility: "Both models flag this as a security/feasibility risk,
...
~/.claude/skills/wstack/bin/wstack-review-log '{"skill":"plan-ceo-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"SELECTIVE_EXPANSION","via":"autoplan","commit":"'"$COMMIT"'"}'

~/.claude/skills/wstack/bin/wstack-review-log '{"skill":"plan-eng-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"FULL_REVIEW","via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope):
```bash
~/.claude/skills/wstack/bin/wstack-review-log '{"skill":"plan-design-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope):
```bash
~/.claude/skills/wstack/bin/wstack-review-log '{"skill":"plan-devex-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"TYPE","tthw_current":"TTHW","tthw_target":"TARGET","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

Dual voice logs (one per phase that ran):
```bash
~/.claude/skills/wstack/bin/wstack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"ceo","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'

~/.claude/skills/wstack/bin/wstack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"eng","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope), also log:
```bash
~/.claude/skills/wstack/bin/wstack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"design","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope), also log:
```bash
~/.claude/skills/wstack/bin/wstack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"dx","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
