...
For high-risk reviews, the reviewer may consult another available local model, review agent, or static analysis tool for cross-validation.
...
Use whatever review surface the current harness exposes. Do not block the review if no second-opinion tool is available.
