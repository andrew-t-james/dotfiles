...
## Independent Second Opinion (Optional)

For high-risk security reviews, the reviewer may consult another available local model, security reviewer, scanner, or static analysis tool for cross-validation.

### Protocol
1. **Form your OWN security analysis FIRST** - Complete the review independently
2. **Consult for validation** - Cross-check findings with an independent reviewer or tool
...
Use whatever review surface the current harness exposes. Do not block the security review if no second-opinion tool is available.
